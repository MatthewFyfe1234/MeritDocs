...
export function OrganisationSlide() {
  ...
  return (
    <SlideLayout
      ...
      subtitle="Two layers. One produces the product. The other enables it."
    >
      <div className="flex-1 min-h-0 flex items-center justify-center">
        <svg viewBox="0 0 700 264" style={{ width: '100%', height: '100%' }}>
          ...
          <text x="264" y="49" textAnchor="middle" fontSize="13" fontWeight="700"
            fill="var(--slide-primary)">Delivery Functions</text>
          <text x="264" y="63" textAnchor="middle" fontSize="9"
            fill="var(--slide-text)" opacity="0.6">The core of the operation.</text>

          {/* Delivery role pills */}
          {deliveryRoles.map(({ label, x }) => (
            <g key={label}>
              <rect x={x} y={78} width={58} height={20} rx="4"
                fill="var(--slide-bg-primary-muted)" stroke="var(--slide-border-primary)" strokeWidth="1" />
              <text x={x + 29} y={92} textAnchor="middle" fontSize="9"
                fill="var(--slide-primary)" opacity="0.85">{label}</text>
            </g>
          ))}

          {/* Inter-role flow arrows */}
          {flowArrowXs.map(x => (
            <line key={x} x1={x + 2} y1={88} x2={x + 14} y2={88}
          ...
          <text x="264" y="179" textAnchor="middle" fontSize="13" fontWeight="700"
            fill="var(--slide-primary)">Support Functions</text>
          <text x="264" y="193" textAnchor="middle" fontSize="9"
            fill="var(--slide-text)" opacity="0.6">The team that enables the delivery functions to operate.</text>

          {/* Support role pills */}
          {supportRoles.map(({ label, x }) => (
            <g key={label}>
              <rect x={x} y={208} width={55} height={20} rx="4"
                fill="white" stroke="var(--slide-border-primary)" strokeWidth="1" opacity="0.8" />
              <text x={x + 27.5} y={222} textAnchor="middle" fontSize="8"
                fill="var(--slide-primary)" opacity="0.85">{label}</text>
            </g>
          ))}
