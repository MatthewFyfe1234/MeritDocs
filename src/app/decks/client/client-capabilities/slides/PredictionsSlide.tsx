...
export function ForecastsSlide() {
  ...
  const total = steps.reduce((sum, s) => sum + s.days, 0); // 18
  ...
  const barWidth = steps.length * stepW + (steps.length - 1) * stepGap; // 600

  const stepCx = (i: number) => startX + i * (stepW + stepGap) + stepW / 2;

  // Bar segment widths proportional to duration
  let segX = barLeft;
  ...
  return (
    <SlideLayout
      ...
      subtitle="Execution data is recorded against each process step. Historical durations generate step-level forecasts that roll up into an end-to-end timeline."
    >
      <div className="flex-1 min-h-0 flex items-center justify-center">
        <svg viewBox="0 0 660 268" style={{ width: '100%', height: '100%' }}>
          ...
          {steps.map((step, i) => {
            ...
            return (
              <g key={step.label}>
                ...
                {/* Arrow to next step — endpoint offset so tip clears the next box edge */}
                {!isLast && (
                  <line
                ...
                {/* Dashed connector to forecast badge */}
          ...
          {/* Total label — centred below the bar */}
